...
export async function up(options: Options): Promise<CommandResult> {
  ...
  if (pid) {
    logger.warn(`Temchik already running [PID: ${pid}]`);
  ...
  if (daemonProcess.pid === undefined) {
    throw new Error("Temchik process not started. PID is undefined.");
  }

  logger.debug(`Temchik daemon started with PID: ${daemonProcess.pid}`);

  if (options.detach) {
    daemonProcess.unref();
    savePid(daemonProcess.pid);
    logger.info(`Temchik started as a daemon [PID: ${daemonProcess.pid}]`);
  } else {
